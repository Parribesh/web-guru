...
function clearAllSessionData(session: Electron.Session) {
  ...
  try {
    ...
    console.log(
      '⚠️ Host resolver cache clear failed:',
...
export class ViewService {
   ...
   */
  static async createBrowserView(
    tab: Tab,
    preloadPath: string,
    mainWindow?: BrowserWindow
  ): Promise<BrowserView> {
    console.log(
      `🔍 Creating BrowserView for tab ${tab.id} with partition: tab-${
        tab.id
      }-${Date.now()}`
    );
    ...
    console.log(
      `📊 BrowserView created with session:`,
      view.webContents.session.getStoragePath()
    );
    console.log(
      `🔒 Session persistence: ${
        view.webContents.session.isPersistent() ? 'PERSISTENT' : 'IN-MEMORY'
      }`
    );
      ...
      setTimeout(() => {
        ...
        if (mainWindow && !mainWindow.isDestroyed() && tab && tab.id) {
          ...
          if (url && url !== 'about:blank') {
            ...
            mainWindow.webContents.send(IPCChannels.tab.update, update);
            console.log(
              '📤 Sent tab:update (did-finish-load) to main window:',
              update.id
            );
          }
        }
      ...
      setTimeout(() => {
        ...
        if (mainWindow && !mainWindow.isDestroyed() && tab && tab.id) {
          ...
          if (url && url !== 'about:blank') {
            ...
            mainWindow.webContents.send(IPCChannels.tab.update, update);
            console.log(
              '📤 Sent tab:update (dom-ready) to main window:',
              update.id,
              'isLoading:',
              update.isLoading
            );
    ...
    view.webContents.on('zoom-changed', (event, zoomDirection) => {
      const currentZoom = view.webContents.getZoomFactor();
      console.log(
        `📢 Zoom-changed event: direction=${zoomDirection}, current=${currentZoom}`
      );
    ...
    view.webContents.on('did-stop-loading', () => {
      ...
      if (
        ...
        eventLogger.success(
          'Navigation',
          `Article fetched: ${title || 'Untitled'}`
        );
      ...
      if (mainWindow && !mainWindow.isDestroyed() && tab && tab.id) {
        ...
        mainWindow.webContents.send('tab:update', update);
        console.log(
          '📤 Sent tab:update (loaded) to main window:',
          update.id,
          'isLoading:',
          update.isLoading
        );
    ...
    view.webContents.on('page-favicon-updated', (event, favicons) => {
      ...
      if (favicons.length > 0) {
        view.webContents.send(IPCChannels.tab.update, {
          ...tab,
          favicon: favicons[0],
        });
      }
